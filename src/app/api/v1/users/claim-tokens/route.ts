...
import { generateAgentToken, generateAgentName } from '@/lib/crypto';
  ...
  return withUserAuth(request, async (auth) => {
    try {
      ...
      const { data, error } = await supabase
        ...
        .select(
          'id, token, name, avatar_url, last_seen_at, last_access_app, created_at, updated_at'
        )
...
export async function POST(request: NextRequest) {
  return withUserAuth(request, async (auth) => {
    try {
      ...
      return successResponse({ token: data }, 201);
    } catch (error) {
      console.error('Create agent error:', error);
      return errorResponse('Failed to create agent', 500);
    }
