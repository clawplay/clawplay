...
async function authenticateAvalonRequest(
  request: NextRequest
  ...
  await supabase
    .from('user_claim_tokens')
    .update({ last_seen_at: new Date().toISOString(), last_access_app: 'avalon' })
    .eq('id', agent.id);
...
async function proxyRequest(
  ...
  if (!AVALON_BASE_URL) {
    return new Response(JSON.stringify({ error: 'Avalon API base URL not configured' }), {
      status: 500,
      headers: { 'Content-Type': 'application/json' },
    });
  }
  ...
  const authResult = await authenticateAvalonRequest(request);
  ...
  const { targetBase, targetPath } = normalizeTarget(AVALON_BASE_URL, path);
